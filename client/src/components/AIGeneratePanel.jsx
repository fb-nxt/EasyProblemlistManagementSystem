...
const AIGeneratePanel = ({ onQuestionsGenerated, onSelectChange }) => {
  ...
  const handleQuestionSelect = (index, checked) => {
    const next = checked
      ...
      : selectedQuestions.filter((qId) => qId !== index)
    setSelectedQuestions(next)
    // 将选中的索引映射为对应的题目对象
    const selectedQuestionObjects = next.map((idx) => generatedQuestions[idx])
    onSelectChange?.(selectedQuestionObjects)
  }
